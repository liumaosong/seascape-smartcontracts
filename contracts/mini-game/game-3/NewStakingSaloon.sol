...
contract NftStaking is Ownable, IERC721Receiver {
    ...
    event GetSignInReward(address owner, uint256 sessionId, uint8 completedNum, uint256 NftId, uint256 imgId);
        ...
        onlyOwner
    {
        ...
        if (_rewardToken != address(0)) {
      	...
      	// sessions[_sessionId] = Session(_totalReward, _period, _startTime, 0, 0, _rewardUnit, 0, 0, _startTime, true, true, 0);//specify nft, burn nft.
        sessions[_sessionId] = Session(_totalReward, _period, _startTime, 0, 0, _rewardUnit, 0, 0, _startTime, false, false, 0);//unspecify nft, do not burn nft.
    ...
    function deposit(uint256 _sessionId, uint8 _index, uint256 _nftId, uint256 _sp, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        nonce++;
      	...
      	{
            // bytes32 _messageNoPrefix = keccak256(abi.encodePacked(_nftId, _sp, msg.sender));//specify&burn nft.
            bytes32 _messageNoPrefix = keccak256(abi.encodePacked(_nftId, _sp));//do not specify&burn nft.
    ...
    function getSignInReward(uint256 _sessionId, uint8 _tagNum, uint8 v, bytes32 r, bytes32 s) external {
        ...
        emit GetSignInReward(msg.sender, _sessionId, _tagNum, NftId, params.imgId);
    }
