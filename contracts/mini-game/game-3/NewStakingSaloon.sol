...
contract NftStaking is Ownable, IERC721Receiver {
    ...
    struct Session {
        ...
        bool burn;             //default true, burn nft
        bool specify;          //default false, Do not specify NFT
    ...
    mapping(uint256 => mapping(address => mapping(uint8 => bool))) public received; //default false, if get a reward changed true
    ...
    event AddSignInReward(uint256 sessionId, uint8 completedNum, uint256 imgId, uint256 generation, uint8 quality, address token, uint256 amount);
    event GetSignInReward(address owner, uint256 sessionId, uint8 completedNum, uint256 NftId, uint256 amount);
    ...
    function giveBonus(uint256 _sessionId, uint256 _bonusPercent) internal returns(bool) {
	    ...
	    uint256 _totalBonus = _interests.mul(MULTIPLIER).mul(_bonusPercent).div(10000).div(MULTIPLIER);
    ...
    function addSignInReward(uint256 _sessionId, bytes calldata data) external onlyOwner{  
        ...
        signinRewards[_sessionId][_tagNum] = Params(imgId, generation, quality, token, amount);
        received[_sessionId][msg.sender][_tagNum] = false;
        
        emit AddSignInReward(_sessionId, _tagNum, imgId, generation, quality, token, amount);
    }
